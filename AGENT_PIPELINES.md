...
## 4. ПАЙПЛАЙН: REVIEW ИЗМЕНЕНИЙ

**Команда**: `npx pythagora --review`

**Файл entry point**: `src/scripts/review.js`

**Описание**: Этот пайплайн **НЕ использует AI/GPT**. Это интерактивный процесс для просмотра и принятия изменений в тестах после повторного запуска приложения.

### 4.1. Краткое Описание

Review процесс детектирует изменения в поведении приложения, сравнивая:
- Оригинальные захваченные тесты (capture)
- Результаты повторного запуска (test)

**Изменения сохраняются в**: `.pythagora/reviewData.json`

**Типы изменений**:
1. Изменения в Response (statusCode, responseData)
2. Изменения в MongoDB запросах (новые, удаленные, измененные)
3. Изменения в параметрах запроса

**Доступные действия**:
- **A** (Accept) - Принять изменения и обновить тест
- **D** (Delete) - Удалить тест
- **S** (Skip) - Пропустить, перейти к следующему
- **R** (Run) - Показать команду для запуска этого теста
- **Q** (Quit) - Выйти из review

### 4.2. Важные Особенности

1. **Не использует AI**: Полностью детерминированная логика сравнения
2. **Интерактивный**: Требует ввода пользователя для каждого изменения
3. **Безопасный**: Не применяет изменения автоматически
4. **Детальный**: Показывает точные различия между capture и test

---

## РЕЗЮМЕ ВСЕХ ПАЙПЛАЙНОВ

| Пайплайн | AI Промпты | Основная Цель | Когда Использовать |
|----------|-----------|---------------|-------------------|
| **1. Генерация Юнит-Тестов** | 1 промпт | Создать тесты с нуля | Нет тестов для функции |
| **2. Расширение Тестов** | 1 промпт | Добавить больше тестов | Есть базовые тесты, нужно больше покрытия |
| **3. Экспорт в Jest** | 4 промпта | Конвертировать captured тесты | После захвата integration тестов |
| **4. Review Изменений** | 0 промптов | Принять/отклонить изменения | После изменения кода приложения |

### Взаимодействие Пайплайнов

```
┌──────────────────────────────────────────────────────────────┐
│                    РАЗРАБОТКА С PYTHAGORA                    │
└──────────────────────────────────────────────────────────────┘

ЭТАП 1: Разработка новой функции
   │
   ├─> Написать функцию (например, calculateTotal)
   │
   └─> ПАЙПЛАЙН #1: Генерация Юнит-Тестов
       └─> npx pythagora --unit-tests --func calculateTotal
           └─> Результат: базовые тесты созданы

ЭТАП 2: Улучшение покрытия
   │
   └─> ПАЙПЛАЙН #2: Расширение Тестов
       └─> npx pythagora --expand-unit-tests --path ./tests/helper.test.js
           └─> Результат: добавлены edge case тесты

ЭТАП 3: Integration тестирование
   │
   ├─> Запустить приложение с Pythagora
   │   └─> npx pythagora --mode capture
   │       └─> Использовать приложение (кликать, делать запросы)
   │           └─> Pythagora захватывает API calls
   │
   └─> ПАЙПЛАЙН #3: Экспорт в Jest
       └─> npx pythagora --export
           └─> Результат: Jest integration тесты созданы

ЭТАП 4: Изменение кода
   │
   ├─> Изменить логику приложения
   │
   ├─> Запустить тесты
   │   └─> npx pythagora --mode test
   │       └─> Обнаружены изменения
   │
   └─> ПАЙПЛАЙН #4: Review Изменений
